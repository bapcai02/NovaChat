...
const HeaderSearchOverlay: React.FC<HeaderSearchOverlayProps> = ({
  open,
  onClose,
  onJumpToMessage,
}) => {
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      const res: any = await apiService.searchMessages(q, undefined, {
        signal: (abortRef.current as AbortController).signal,
      } as any);
      const data = res?.data ?? [];
      const mappedMessages: SearchResult[] = (
      ...
      const combined = mappedMessages.filter((r) =>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 mt-6 w-[92%] max-w-2xl">
        <div className="rounded-xl bg-white shadow-2xl ring-1 ring-black/5 p-3">
          ...
          <div className="flex items-center gap-2 px-1 pt-2">
            {(["all", "message", "user", "channel"] as FilterType[]).map(
              (f) => (
                <button
                  key={f}
                  onClick={(e) => {
