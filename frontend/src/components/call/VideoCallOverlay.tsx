  ...
  if (minimized) {
    const onMouseDown = (e: React.MouseEvent) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        posRef.current = {
          x: Math.max(8, ev.clientX - dx),
          y: Math.max(8, ev.clientY - dy),
        };
    ...
    return (
      ...
      >
        <div
          className="flex items-center justify-between px-3 py-2 border-b border-gray-100 cursor-move"
          onMouseDown={onMouseDown}
        >
          <div>
            <div className="text-xs font-medium text-gray-900">
              {calleeName || 'Video Call'}
            </div>
            {statusText && (
          ...
          <div className="flex items-center gap-1">
            {onToggleMinimize && (
              <button
                ...
                className="h-7 px-2 text-xs rounded bg-gray-100 hover:bg-gray-200"
              >
                Mở rộng
              </button>
            )}
            <button
              type="button"
              onClick={onHangup}
              className="h-7 px-2 text-xs rounded bg-red-600 text-white hover:bg-red-700"
            >
              Kết thúc
            </button>
          </div>
        ...
        <div className="relative bg-black w-full h-[calc(200px-40px)]">
          ...
          <video
            ref={localRef}
            className="absolute bottom-2 right-2 w-24 h-16 bg-black rounded border border-white/20 object-cover"
            autoPlay
            muted
            playsInline
          />
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-3xl mx-4 overflow-hidden">
        ...
        <div className="relative bg-black">
          ...
          <video
            ref={localRef}
            className="absolute bottom-4 right-4 w-48 h-32 bg-black rounded-md border border-white/20"
            autoPlay
            muted
            playsInline
          />
