export type AudioCallState = 'idle' | 'calling' | 'ringing' | 'in_call' | 'ended' | 'error'
...
export function useAudioCall({ conversationId, currentUserId, stunUrls = ['stun:stun.l.google.com:19302'], turnConfig = null }: UseAudioCallOptions) {
  ...
  const getIceServers = (): RTCIceServer[] => {
    ...
    if (turnConfig) servers.push({ urls: turnConfig.urls, username: turnConfig.username, credential: turnConfig.credential })
    return servers
  }
  ...
  const initPeer = async () => {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) {
        ...
        if (ws) {
          ws.send({ type: 'rtc_candidate', conversation_id: conversationId, from: currentUserId, candidate: e.candidate })
        }
      }
    }
    pc.ontrack = (e) => {
  ...
  const answer = async (remoteSdp: any) => {
    try {
      ...
      const ws = (window as any).getWebSocketClient?.() || null
      if (ws) {
        ws.send({ type: 'rtc_answer', conversation_id: conversationId, from: currentUserId, sdp: ans })
      }
      state = 'in_call'
    } catch (e) {
  ...
  const hangup = async () => {
    try {
      const ws = (window as any).getWebSocketClient?.() || null
      if (ws) {
        ws.send({ type: 'rtc_end', conversation_id: conversationId, from: currentUserId })
      }
      if (pc) {
