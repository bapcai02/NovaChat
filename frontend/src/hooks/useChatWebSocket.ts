...
export function attachChatWsHandlers(handlers: ChatWsHandlers) {
  ...
  wsClient.onMessage((message: WebSocketMessage) => {
    if (message.type === "connected" && (message as any).online_users) {
      handlers.onConnectedOnlineUsers?.(
        normalizeIds((message as any).online_users),
      );
      return;
    }
    if (
      ...
      handlers.onSubscribeAllOnlineUsers?.(
        normalizeIds((message as any).online_users),
      );
      return;
    }
    ...
    if (message.type === "message_read") {
      ...
      if (convId && userId && messageId)
        handlers.onMessageRead?.(convId, userId, messageId);
      return;
    }
